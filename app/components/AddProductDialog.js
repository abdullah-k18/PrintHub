...
export default function AddProductDialog({ open, onClose, onAdd, uid }) {
    ...
    const handleAddProduct = async () => {
        if (images.length === 0) {
            toast.error("Please upload at least one image before adding the product.");
            return;
        }
        ...
        await Promise.all(
            images.map(async (image, index) => {
                ...
                const uniqueFileName = `product-${Date.now()}-${index + 1}-${file.name}`;
